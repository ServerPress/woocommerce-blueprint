 ...
 * Description: Fetches and installs the latest version of WordPress, WooCommerce, compatible with DesktopServer 5.X only.
 * Version: 1.0.1
...
$ds_runtime->add_action('ds_workflow_create_done', function($results) {
	...
	$cmd = "wp plugin install child-theme-configurator --activate";
	$ds_runtime->exec($cmd, $sitePath);

	//** Download & Activate StoreFront Theme
	$cmd = "wp theme install storefront --activate";
	$ds_runtime->exec($cmd, $sitePath);
	ds_cli_exec( "wp theme install storefront --activate");

	//** Download GeoIP.dat and place in the uploads folder
	$cmd = "wget http://geolite.maxmind.com/download/geoip/database/GeoLiteCountry/GeoIP.dat.gz && gunzip GeoIP.dat.gz && mv GeoIP.dat ./wp-content/uploads/";
	$ds_runtime->exec($cmd, $sitePath);
